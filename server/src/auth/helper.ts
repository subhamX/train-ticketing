...
export const generateToken = async (username: string) => {
    ...
    const refreshtoken = jwt.sign(payload, process.env.JWT_REFRESH_SECRET as jwt.Secret, {
        algorithm: "HS256",
    });
    await db.query(
        `UPDATE users SET current_token = $1 WHERE username = $2`,
...
export const verifyToken = async (req: Request , res: Response, next: any) => {
    ...
    try {
        const verifiedData: any = jwt.verify(token, process.env.JWT_SECRET as jwt.Secret);
        ...
        if (err.name === 'JsonWebTokenError') {
            ...
            try {
                ...
                let instance: jwtpayload = jwt.verify(refresh_token, process.env.JWT_REFRESH_SECRET as jwt.Secret) as jwtpayload;
                let username = instance['username'];
                let queryResponse = await db.query('SELECT current_token FROM users where username=$1', [username]);
